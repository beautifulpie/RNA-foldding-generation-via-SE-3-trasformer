...
class Molecule3D(InMemoryDataset):
    ...
    def process(self):
        ...
        suppl_list = [
            Chem.SDMolSupplier(p, removeHs=False, sanitize=True)
            for p in sdf_paths
        ]
            ...
            with Pool(processes=120) as pool:
                ...
                for j, graph in tqdm(enumerate(iter), total=len(suppl)):
                    ...
                    data.y = torch.FloatTensor(
                        [target_df.iloc[abs_idx, 6]]
                    ).unsqueeze(1)

                    data.pos = torch.tensor(
                        graph["position"], dtype=torch.float32
                    )
    ...
    def get_idx_split(self, split_mode="random"):
        ...
        split_dict = json.load(
            open(osp.join(self.raw_dir, f"{split_mode}_split_inds.json"), "r")
        )
    ...
    def mol2graph(self, mol):
        ...
        for atom in mol.GetAtoms():
            atom_features_list.append(utils.atom_to_feature_vector(atom))
        ...
        if len(mol.GetBonds()) > 0:  # mol has bonds
            ...
            for bond in mol.GetBonds():
                ...
                edge_feature = utils.bond_to_feature_vector(bond)
